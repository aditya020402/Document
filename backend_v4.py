...
class WorkflowState(TypedDict):
    ...
    messages: List[Any]
    current_step: str
    workflow_mode: str  # 'full_automation' or 'content_improvement'
...
class DocumentAutomationWorkflow:
    ...
    def _build_content_workflow(self) -> StateGraph:
        """Content quality workflow - NEW DEDICATED AGENTS"""
        workflow = StateGraph(WorkflowState)

        workflow.add_node("parse_document", self._parse_document_agent)
        workflow.add_node("clean_text", self._text_cleaning_agent)
        workflow.add_node("analyze_content_quality", self._content_quality_scoring_agent)  # NEW
        workflow.add_node("improve_content", self._content_improvement_agent)  # NEW
    ...
    # ----------------------
    # NEW AGENT 3A: Content Quality Scoring Agent (for content improvement mode)
    # ----------------------
    ...
    async def _content_quality_scoring_agent(self, state: WorkflowState) -> WorkflowState:
        ...
        # NEW: 6-dimensional content quality analysis
        ...
        # Mark automation-specific fields as skipped
    ...
    async def _analyze_content_quality_dimensions(self, text: str) -> Dict[str, Any]:
        """
        NEW: Analyze document quality across 6 dimensions:
        1. Clarity - how easy to understand
        2. Completeness - how thorough and comprehensive
        3. Accuracy - factual correctness and precision
        4. Consistency - uniform terminology and style
        5. Readability - ease of reading and comprehension
        6. Organization - logical structure and flow
        """
        ...
        for idx, chunk in enumerate(chunks):
            ...
            **Analyze the CONTENT naturally, focusing on document quality, NOT automation feasibility.**

            Return JSON:
            {{
                "clarity_score": <0-10>,
                "clarity_reasoning": "<why this score for clarity>",
                "completeness_score": <0-10>,
                "completeness_reasoning": "<why this score for completeness>",
                "accuracy_score": <0-10>,
                "accuracy_reasoning": "<why this score for accuracy>",
                "consistency_score": <0-10>,
                "consistency_reasoning": "<why this score for consistency>",
                "readability_score": <0-10>,
                "readability_reasoning": "<why this score for readability>",
                "organization_score": <0-10>,
                "organization_reasoning": "<why this score for organization>",
        ...
        Provide:
        1. Top 5 improvement recommendations for document quality
        ...
        try:
            ...
            recommendations = {
                "improvement_recommendations": ["Improve clarity of language", "Add missing details", "Enhance organization"],
                "critical_issues": ["Vague terminology", "Missing sections"],
                "quick_wins": ["Fix typos", "Add headings", "Clarify ambiguous statements"]
            }
    ...
    async def _extract_content_quality_issues(self, text: str) -> Dict[str, Any]:
        """Extract specific quality issues from content"""
        chunks = chunk_text_by_paragraphs(text, max_chunk_size=4000, overlap_paragraphs=1)
        aggregated = {
            "vague_terms": [],
            "unclear_statements": [],
        ...
        for idx, chunk in enumerate(chunks):
            prompt = f"""
            Analyze this document section for content quality issues (NOT automation issues):

            - "vague_terms": ambiguous or imprecise terminology
            - "unclear_statements": confusing or poorly worded sentences
            - "missing_details": places needing more information or explanation
        ...
        # Generate remediation for vague terms
    ...
    async def _generate_vague_term_remediation(self, vague_terms: List[str], document_text: str) -> List[Dict[str, str]]:
        """Generate specific remediation for each vague term"""
        if not vague_terms:
            return []
        ...
        for i in range(0, len(vague_terms), batch_size):
            ...
            [
              {{
                ...
                "suggested_replacement": "<specific clear alternative>",
            ...
            try:
                ...
                for term in batch:
                    remediation_list.append({
                        "vague_term": term,
                        "why_vague": "This term is unclear and imprecise",
                        "suggested_replacement": "Replace with specific, clear language",
    ...
    def _calculate_content_quality_score(self, quality_scores: Dict, quality_issues: Dict) -> Dict[str, Any]:
        ...
        penalty = min(2.0, total_issues * 0.1)  # Max 2 points penalty
    ...
    # ----------------------
    # NEW AGENT 4A: Content Improvement Agent (for content improvement mode)
    # ----------------------
    ...
    async def _generate_improved_content_for_quality(self, original_text: str, quality_scores: Dict) -> Dict[str, Any]:
        """Generate improved content focusing on general document quality"""
        ...
        for i, chunk in enumerate(chunks):
            ...
            - Organization: {quality_scores.get('organization_score', 5)}/10

            Focus on:
            - Make language clearer and more precise
            - Add missing details and context
            - Improve sentence structure and flow
            - Enhance organization and coherence
            - Fix any ambiguities or unclear statements

            **IMPORTANT: Keep [IMAGE_X_ANALYSIS] blocks intact.**
    ...
    # ----------------------
    # EXISTING AUTOMATION AGENTS (UNCHANGED - keep all previous automation agents)
    # ----------------------
    # ... (Keep all existing automation analysis agents exactly as they were)

    async def _enhanced_automation_analysis_agent(self, state: WorkflowState) -> WorkflowState:
        # Keep existing automation agent unchanged
    ...
    # ... (Keep all other existing automation agent methods unchanged)
    # _ai_runbook_feature_extraction, _extract_rule_data, 
    # _perform_five_metric_analysis_on_full_content,
    # _generate_improved_actual_document, _calculate_overall_score,
    # _similarity_matching_agent, _automation_commands_agent, _script_generation_agent
    # (I'm skipping the repetition of these as they remain 100% identical to your previous version)

    # ----------------------
    # Public entrypoints with workflow selection
    # ----------------------

    async def process_document(self, pdf_path: str, workflow_mode: str = 'full_automation') -> Dict[str, Any]:
        """
        Process PDF documents with workflow mode selection
        
        Args:
            pdf_path: Path to PDF file
            workflow_mode: 'full_automation' or 'content_improvement'
        """
    ...
    async def process_text_document(self, text_file_path: str, workflow_mode: str = 'full_automation') -> Dict[str, Any]:
        """Process plain text documents with workflow mode selection"""
