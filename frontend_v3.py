...
if st.session_state.workflow_mode == 'full_automation':
    ...
    **📝 Content Quality Improvement Mode**
    - Extract and clean document content
    - Identify vague terms with remediation suggestions
    - Detect unclear statements and missing details
    - Generate improved, clearer document version
    
    *Best for: General documents, policies, procedures needing clarity improvements*
    """)
...
if st.session_state.results:
    ...
    # NEW: Dynamic tab creation based on workflow mode
    ...
    with tab3:
        if workflow_mode == 'content_improvement':
            ...
            quality_data = analysis.get('quality_data', {})
            
            # Quality Issues Summary
            st.markdown("#### 📊 Content Quality Issues Found")
            col1, col2, col3, col4 = st.columns(4)
            
            with col1:
                vague_count = len(quality_data.get('vague_terms', []))
                st.metric("Vague Terms", vague_count, delta=None, delta_color="inverse")
            with col2:
                unclear_count = len(quality_data.get('unclear_statements', []))
                st.metric("Unclear Statements", unclear_count, delta=None, delta_color="inverse")
            with col3:
                missing_count = len(quality_data.get('missing_details', []))
                st.metric("Missing Details", missing_count, delta=None, delta_color="inverse")
            with col4:
                inconsistent_count = len(quality_data.get('inconsistencies', []))
                st.metric("Inconsistencies", inconsistent_count, delta=None, delta_color="inverse")
            
            total_issues = quality_data.get('total_issues', 0)
            if total_issues == 0:
                st.success("🎉 Excellent! No significant content quality issues found!")
            elif total_issues < 5:
                st.info(f"✅ Good content quality with only {total_issues} minor issues")
            elif total_issues < 15:
                st.warning(f"⚠️ Moderate content quality issues found ({total_issues} issues)")
            else:
                st.error(f"❌ Significant content quality issues found ({total_issues} issues)")
            ...
            with col1:
                ...
                vague_term_remediation = quality_data.get('vague_term_remediation', [])
                ...
                unclear_statements = quality_data.get('unclear_statements', [])
            ...
            with col2:
                ...
                missing_details = quality_data.get('missing_details', [])
                if missing_details:
                ...
                inconsistencies = quality_data.get('inconsistencies', [])
                if inconsistencies:
            ...
            # EXISTING: Full Automation Analysis Tab (UNCHANGED)
    ...
    with tab4:
        if workflow_mode == 'content_improvement':
            # NEW: Improved Document for Content Mode
            ...
            if improved:
                ...
                with col2:
                    ...
                    quality_score = improved.get('content_quality_score', 5)
                    st.metric("Quality Score", f"{quality_score}/10")
                    
                    st.markdown("**Improvements:**")
                    improvement_count = len(improved.get('improvements_made', []))
                    st.metric("Changes Made", improvement_count)
            ...
            # EXISTING: Similar Automations Tab (UNCHANGED)
