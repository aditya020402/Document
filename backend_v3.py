...
class WorkflowState(TypedDict):
    ...
    messages: List[Any]
    current_step: str
    workflow_mode: str  # NEW: 'full_automation' or 'content_improvement'
...
class DocumentAutomationWorkflow:
    ...
    # ----------------------
    # NEW: Two separate workflow graphs
    # ----------------------
    ...
    def _build_content_workflow(self) -> StateGraph:
        """Content improvement workflow - only parsing, cleaning, and analysis"""
        workflow = StateGraph(WorkflowState)

        workflow.add_node("parse_document", self._parse_document_agent)
        workflow.add_node("clean_text", self._text_cleaning_agent)
        workflow.add_node("analyze_content", self._content_quality_analysis_agent)  # NEW

        workflow.add_edge(START, "parse_document")
        workflow.add_edge("parse_document", "clean_text")
        workflow.add_edge("clean_text", "analyze_content")
    ...
    async def _extract_ai_key_phrases(self, text: str) -> List[str]:
        ...
        for i, chunk in enumerate(chunks):
            ...
            most important (technical terms, process names, tools,
            commands, systems, workflows, concepts, visual elements from images).
    ...
    # ----------------------
    # NEW AGENT 3A: Content Quality Analysis (for content improvement mode)
    # ----------------------

    async def _content_quality_analysis_agent(self, state: WorkflowState) -> WorkflowState:
        print("📝 Content Quality Analyzer...")
        cleaned_text = state['cleaned_content']['cleaned_text']

        # Extract quality issues
        quality_data = await self._extract_content_quality_issues(cleaned_text)
        
        # Generate improved document
        improved_document = await self._generate_improved_content(cleaned_text, quality_data)
        ...
        # Mark similarity and script as skipped
        ...
        state['current_step'] = 'content_analysis_complete'
        return state

    async def _extract_content_quality_issues(self, text: str) -> Dict[str, Any]:
        """Extract vague terms, clarity issues, and content problems"""
        chunks = chunk_text_by_paragraphs(text, max_chunk_size=4000, overlap_paragraphs=1)
        aggregated = {
            "vague_terms": [],
            "unclear_statements": [],
        ...
        for idx, chunk in enumerate(chunks):
            ...
            - "vague_terms": unclear/ambiguous phrases that need clarification
            - "unclear_statements": confusing or poorly worded sentences
            - "missing_details": places where more information is needed
            ...
            try:
                ...
                try:
                    ...
                    parsed = {"vague_terms": [], "unclear_statements": [], "missing_details": [], "inconsistencies": []}
        ...
        return {
            ...
            'total_issues': sum(len(v) if isinstance(v, list) else 0 for k, v in aggregated.items())
        }

    async def _generate_improved_content(self, original_text: str, quality_data: Dict) -> Dict[str, Any]:
        """Generate improved content focusing on clarity and completeness"""
        ...
        for i, chunk in enumerate(chunks):
            prompt = f"""
            Improve this document section for clarity, completeness, and precision.

            Focus on:
            - Replace vague terms with specific language
            - Clarify unclear statements
            - Add missing details where evident
            - Fix inconsistencies
        ...
        return {
            ...
            "content_quality_score": max(1, 10 - (quality_data['total_issues'] // 5))
        }
    ...
    async def _generate_vague_term_remediation(self, vague_terms: List[str], document_text: str) -> List[Dict[str, str]]:
        ...
        for i in range(0, len(vague_terms), batch_size):
            ...
            1. Why it's vague
            2. A specific, clear replacement
            3. Context-aware example
            ...
            try:
                ...
                for term in batch:
                    remediation_list.append({
                        "vague_term": term,
                        "why_vague": "This term is unclear and not specific",
                        "suggested_replacement": "Replace with specific, measurable criteria",
                        "example": f"Instead of '{term}', specify exact conditions",
                        "severity": "Medium"
                    })
    ...
    # ----------------------
    # EXISTING AGENT 3B: Full Automation Analysis (unchanged)
    # ----------------------
    # (Keep all the existing automation analysis code: _enhanced_automation_analysis_agent, 
    # _ai_runbook_feature_extraction, _extract_rule_data, _perform_five_metric_analysis_on_full_content, 
    # _generate_improved_actual_document, _calculate_overall_score)
    
    # I'll include the key ones but keep them the same as before:
    ...
    # [Keep all other existing agents: similarity, commands, script generation - UNCHANGED]
    # ... (I'll skip repeating them here for brevity, but they remain identical)
    ...
    # Keep all similarity, commands, script agents UNCHANGED from before
    # (Similarity matching, automation commands, script generation - identical to previous version)
    ...
    # ----------------------
    # NEW: Public entrypoints with workflow selection
    # ----------------------
    ...
    async def process_text_document(self, text_file_path: str, workflow_mode: str = 'full_automation') -> Dict[str, Any]:
        ...
        if workflow_mode == 'content_improvement':
            initial_state = await self._content_quality_analysis_agent(initial_state)
        else:
