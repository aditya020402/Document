...
if st.session_state.results:
    ...
    with tab1:
        ...
        # Image Analysis Display (RESTORED)
    ...
    with tab2:
        ...
        # Image Analysis Summary (RESTORED)
        if input_mode == 'Upload PDF File' and 'image_analysis' in results['cleaned_content'] and results['cleaned_content']['image_analysis']:
            st.divider()
            st.markdown("#### 🖼️ Multimodal Image Analysis Results")
        ...
        # Structured Content Sections (RESTORED)
        if 'structured_content' in results['cleaned_content']:
            ...
            # Data Definitions
            if structured.get('sections', {}).get('data_definitions'):
                st.markdown("##### 📊 Data Definitions (Complete)")
                data_definitions = structured['sections']['data_definitions']
                st.info(f"Found {len(data_definitions)} data definitions")
                with st.expander(f"📋 View All {len(data_definitions)} Data Definitions", expanded=False):
                    for i, data in enumerate(data_definitions, 1):
                        st.markdown(f"**{i}.** {data}")
                        if i < len(data_definitions):
                            st.markdown("---")
            
            # Decision Rules
            if structured.get('sections', {}).get('decision_rules'):
                st.markdown("##### 🔀 Decision Rules (Complete)")
                decision_rules = structured['sections']['decision_rules']
                st.info(f"Found {len(decision_rules)} decision rules")
                with st.expander(f"📋 View All {len(decision_rules)} Decision Rules", expanded=False):
                    for i, rule in enumerate(decision_rules, 1):
                        st.markdown(f"**{i}.** {rule}")
                        if i < len(decision_rules):
                            st.markdown("---")
    ...
    # TAB 3: Automation Analysis (WITH QUALITY FLAGS)
    with tab3:
        ...
        with col1:
            ...
            with st.expander(f"📋 View {vague_count} Vague Terms", expanded=(vague_count > 0)):
                ...
                if vague_terms:
                    st.warning("⚠️ These terms are unclear and should be made more specific for automation:")
